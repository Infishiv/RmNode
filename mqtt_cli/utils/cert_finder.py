...
def convert_unix_path_to_windows(unix_path: str, base_path: str) -> str:
    ...
    try:
        # Extract the relative path after esp-rainmaker-certs
        if 'esp-rainmaker-certs' in unix_path:
            relative_path = unix_path.split('esp-rainmaker-certs/')[-1]
...
def find_node_cert_key_pairs(base_path: str, is_rainmaker: bool = False) -> List[Tuple[str, str, str]]:
    ...
    if not Path(base_path).exists():
        raise FileNotFoundError(f"Certificate directory not found: {base_path}")
...
def get_cert_and_key_paths(base_path: str, node_id: str, is_rainmaker: bool = False) -> Tuple[str, str]:
    ...
    Raises:
        FileNotFoundError: If certificates are not found or base_path doesn't exist
    ...
    for nodeID, cert_path, key_path in node_pairs:
        if str(nodeID) == str(node_id):
            # Verify files still exist (in case they were removed after initial search)
    ...
    raise FileNotFoundError(f"Certificate and key not found for node {node_id} in {base_path}")
...
def get_root_cert_path(config_dir: Path) -> str:
    ...
    Raises:
        FileNotFoundError: If root certificate is not found
    """
    if not config_dir:
    ...
    raise FileNotFoundError(
        "Root CA certificate not found. Please ensure root.pem exists in either:\n"
        f"1. {config_dir}/certs/root.pem\n"
        f"2. {package_root}/certs/root.pem"
    )
