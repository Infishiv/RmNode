...
@config.command('set-cert-cli')
@click.option('--path', required=True, help='Path to Node\'s Certs directory')
@click.option('--update/--no-update', default=True, 
              help='Update existing nodes if they exist (default: True)')
@click.pass_context
def set_cert_cli(ctx, path, update):
    """Set the Node's Certs path and discover nodes.
    ...
    try:
        ...
        config_manager = ConfigManager(ctx.obj['CONFIG_DIR'])
        config_manager.set_cert_cli_path(str(path))
        ...
        # Print results
        click.echo(click.style(f"✓ Node's Certs path set to: {path}", fg='green'))
        if new_nodes:
        ...
        click.echo(click.style(f"✗ Failed to set Node's Certs path: {str(e)}", fg='red'), err=True)
        raise click.Abort()

@config.command('get-cert-cli')
@click.pass_context
def get_cert_cli(ctx):
    """Get the Node's Certs path.
    ...
    try:
        config_manager = ConfigManager(ctx.obj['CONFIG_DIR'])
        path = config_manager.get_cert_cli_path()
        if path:
            click.echo(f"Node's Certs path: {path}")
        else:
            click.echo("Node's Certs path not set")
    except Exception as e:
        click.echo(click.style(f"✗ Error: {str(e)}", fg='red'), err=True)
        raise click.Abort()
